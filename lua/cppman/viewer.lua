...
local function get_pager_script()
  ...
  local res = vim.system(
    { "python3", "-c", "import cppman, os; print(os.path.join(os.path.dirname(cppman.__file__), 'lib', 'pager.sh'))" },
    { text = true }
  ):wait()
  if res.code == 0 then
...
local function get_cursor_lookup_text()
  ...
  local ref = line:match("^(.-)%s*%(%d+[a-zA-Z]*%)%s*%b[]%s*$")
    or line:match("^(.-)%s*%(%d+[a-zA-Z]*%)%s*$")
